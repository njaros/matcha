...
def add_message(**kwargs):
    ...
    return chat_sql.insert_message(
        data={
...
def get_message_list_by_room_id(**kwargs):
    return chat_sql.get_message_list_by_room_id(
        (uuid.isUuid(request.json["room_id"]))
    )


@token_required
def increment_unread_msg_count(**kwargs):
    chat_sql.increment_unread_msg_count(
        kwargs["user"]["id"], uuid.isUuid(request.json["room_id"])
    )
    return [], 200
...
def set_unread_msg_count_to_0(**kwargs):
    chat_sql.set_unread_msg_count_to_0(
        kwargs["user"]["id"], uuid.isUuid(request.json["room_id"])
    )
    return [], 200
...
def get_unread_msg_count(**kwargs):
    count = chat_sql.get_unread_msg_count(
        kwargs["user"]["id"], uuid.isUuid(request.json["room_id"])
    )
    if count is None:
